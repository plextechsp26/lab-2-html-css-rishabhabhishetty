...
*You won't be able to see some of these changes reflected in your web page, why?*
...
HTML also provides **semantic elements** that work just like `<div>` but carry meaning about what they represent on the page. Using them makes your code easier to read and understand:
- `<header>` - The top section of a page, typically containing a title or navigation
- `<nav>` - A section containing navigation links
- `<main>` - The primary content area of the page
- `<footer>` - The bottom section of a page, often containing credits or links

Semantically, `<header>My Site</header>` and `<div>My Site</div>` behave identically in a browser, but the former makes it immediately clear what that section is for.
...
- Use flexboxes for positioning. A flexbox is a container that automatically arranges its children in a row or column. You enable it with `display: flex` on the parent element. Here are the key properties you'll use:
  - `flex-direction` - Sets the direction children are arranged: `row` (default, left to right) or `column` (top to bottom)
  - `justify-content` - Aligns children along the main axis (e.g. `flex-start`, `flex-end`, `center`, `space-between`)
  - `align-items` - Aligns children along the cross axis (e.g. `center` to vertically center items in a row)
  - `gap` - Sets the spacing between children (e.g. `gap: 10px`)
  - `flex-wrap` - Controls whether children wrap to a new line when there's no more space (`wrap` or `nowrap`)
  - `flex` / `flex-grow` - Controls how much a child grows to fill available space
  
  Check out the full guide here: [Guide to FlexBoxes](https://css-tricks.com/snippets/css/a-guide-to-flexbox/). You will use this more later.
...
Inside of your `<body>`, you are able to add `<script>` tags to embed client side JavaScript code. These can then be added to certain elements of your page as event listeners that will call the function when the event occurs. For instance, consider the following block of code:
...
All CSS files end with the `.css` extension. These files will look very different from a lot of other code you might have written. These style sheets consist of a series of selectors, which you use to choose what to apply your styling to, and the actual styling. So the file will look something like this:
...
The `p` selector selects all `<p>` tags and applies the styling to those elements. If you want to apply the same styling to multiple tags, you can separate the selectors with a comma (if you want to also apply it to `h1`, the selector would be `p, h1`). Note that a space between selectors means something different — `p h1` selects `h1` elements that are *descendants* of a `p`, not both independently. 

There is also the `#example` selector, which selects the single element with `id="example"`. IDs are meant to be unique — only one element on the page should have a given ID, while many elements can share a class.
